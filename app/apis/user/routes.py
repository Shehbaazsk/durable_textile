...
@user_router.get("/", status_code=status.HTTP_200_OK, dependencies=[Depends(has_role('ADMIN'))])
def list_users(filters: UserFilters = Depends(),
               sort_by: list[UserSortEnum] = Query(
                   default=UserSortEnum.desc_created_at),
